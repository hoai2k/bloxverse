...
import { GamepadManager, KeyboardSource, gamepadsAvailable } from './gamepad.js';
import { VERSION } from './version.js';
import { enchantLevel, rollEnchantments, applyEnchant } from './enchant.js';
import { SEA_LEVEL } from './worldgen.js';

function stackOf(s, n) { const o = { id: s.id, count: n, dmg: s.dmg }; if (s.ench) o.ench = { ...s.ench }; return o; }
...
class Input {
  constructor(game) {
    ...
    const c = game.canvas;
    document.addEventListener('keydown', (e) => { if (e.target.tagName === 'INPUT') return; if (!this.keys.has(e.code)) this.justPressed.add(e.code); this.keys.add(e.code); if (game.running && !game.anyScreenOpen() && ['Space', 'Tab', 'KeyW', 'KeyA', 'KeyS', 'KeyD'].includes(e.code)) e.preventDefault(); });
    ...
    c.addEventListener('mousedown', (e) => { if (!game.running) return; if (!this.locked) { game.requestPointerLock(); return; } this.buttons.add(e.button); game.onMouseDown(e.button, game.player); e.preventDefault(); });
    ...
    document.addEventListener('pointerlockchange', () => { if (game.touch) return; this.locked = document.pointerLockElement === c; if (!this.locked && game.running && !game.anyScreenOpen() && !game.player.dead) { /* user pressed Esc: open pause */ game.ui.openPause(); } });
  }
...
export class Game {
  constructor(canvas, settings) {
    ...
    this.fps = 0; this.frameMs = 0; this.sunLevel = 1; this.stats = {}; this.daysSinceSleep = 0; this.lastSleepDay = 0;
    this.worlds = new Map(); this.bossEntity = null; this.rng = mulberry32(Date.now() >>> 0);
    this.players = []; this.version = VERSION;
  }
  requestPointerLock() { if (this.touch) { this.input.locked = true; return; } if (this.running && !this.anyScreenOpen()) { try { const r = this.canvas.requestPointerLock(); if (r && r.catch) r.catch(() => { }); } catch { } } }
  ...
  async start(meta) {
    ...
    if (!this.renderer) { this.renderer = this.panoramaRenderer || new Renderer(this.canvas); this.input = new Input(this); this.pads = new GamepadManager(this); attachGame(this); if (hasTouch()) { this.touch = new TouchControls(this); } }
    ...
    // Player 1 always uses keyboard + mouse (or touch); extra players join with gamepads.
    this.players = []; this.renderer.setViewCount(0);
    if (!this.ui) { this.ui = new UI(this); }
    this.ui.reset();
    const p1 = this.addPlayer(new KeyboardSource(this.input, this), meta.player);
    this.player = p1;
    this.applySettings(this.settings);
    if (Array.isArray(meta.extraPlayers)) {
      const pads = this.pads.list();
      meta.extraPlayers.forEach((data, i) => { if (pads[i]) this.addPlayer(pads[i], data); });
    }
  ...
  // ---------- players ----------
  addPlayer(source, saved = null) {
    if (this.players.length >= 4) return null;
    const idx = this.players.length;
    const p = new Player(this, idx, source);
    p.world = this.world;
    p.view = this.renderer.createView();
    this.players.push(p);
    if (source && source.kind === 'gamepad') { p.usesGamepad = true; if (source.suppressInput) source.suppressInput(4); }
    this.ui.addPlayerHud(p);
    if (saved) p.deserialize(saved);
    else {
      p.setGamemode(this.meta.gamemode || 'survival');
      const host = this.players[0];
      if (idx > 0 && host) { p.x = host.x + (Math.random() - 0.5) * 2; p.z = host.z + (Math.random() - 0.5) * 2; p.y = host.y; p.spawn = host.spawn; this.settleSpawn(p); }
      else { p.x = 0.5; p.z = 0.5; p.y = 90; }
    }
    this.renderer.layoutViews(); this.ui.layoutHuds();
    if (idx > 0) { this.ui.showTitle('', p.name + ' joined', 2.5); this.audio.play('levelup', { volume: 0.5 }); }
    return p;
  }
  removePlayer(p) {
    const i = this.players.indexOf(p); if (i <= 0) return; // player 1 can't leave
    if (p.screen) this.ui.closeScreen(p, true);
    for (const s of p.inventory.slots) if (s) this.entities.dropItem(p.x, p.y + 0.5, p.z, s, true);
    this.renderer.removeView(p.view); this.ui.removePlayerHud(p);
    if (p.model) { this.renderer.scene.remove(p.model); p.model = null; }
    this.players.splice(i, 1);
    this.players.forEach((pl, n) => { pl.index = n; pl.name = n === 0 ? 'Player' : 'Player ' + (n + 1); });
    this.renderer.layoutViews(); this.ui.layoutHuds();
    this.ui.showTitle('', 'A player left', 2);
  }
  eachPlayer(fn) { for (const p of this.players) fn(p); }
  livePlayers() { return this.players.filter(p => !p.dead && p.gamemode !== 'spectator'); }
  nearestPlayer(x, y, z, opts = {}) {
    let best = null, bd = Infinity;
    for (const p of this.players) {
      if (p.dead && !opts.includeDead) continue;
      if (p.gamemode === 'spectator' && !opts.includeSpectators) continue;
      if (opts.notCreative && p.gamemode === 'creative') continue;
      const d = (p.x - x) ** 2 + (y !== undefined ? (p.y - y) ** 2 : 0) + (p.z - z) ** 2;
      if (d < bd) { bd = d; best = p; }
    }
    return best;
  }
  nearestPlayerDist(x, y, z) { const p = this.nearestPlayer(x, y, z, { includeSpectators: true, includeDead: true }); return p ? Math.hypot(p.x - x, p.y - y, p.z - z) : Infinity; }
  anyScreenOpen() { return this.players.some(p => p.screenOpen) || (this.ui && this.ui.globalScreen); }
  get splitScreen() { return this.players.length > 1; }
  hurtPlayerObj(player, amount, source, bypassArmor = false) { return player.hurt(amount, source, bypassArmor ? { fall: true } : {}); }
  ...
  placeBonusChest() { const w = this.world, p = this.player; const x = Math.floor(p.x) + 2, z = Math.floor(p.z); let y = w.surfaceY(x, z) + 1; while (y > 1 && (BLOCKS[w.getBlock(x, y - 1, z)].leaves || !BLOCKS[w.getBlock(x, y - 1, z)].solid)) y--; w.setBlock(x, y, z, B.chest, 0); const te = w.getTileEntity(x, y, z); te.slots = new Array(27).fill(null); const items = [[B.oak_log, 6], [I.wooden_pickaxe, 1], [I.wooden_axe, 1], [I.bread, 4], [I.apple, 3], [B.torch, 8], [B.oak_planks, 12], [I.stick, 6]]; items.forEach(([id, n], i) => te.slots[i * 3] = makeStack(id, n)); for (const [dx, dz] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) if (w.getBlock(x + dx, y, z + dz) === 0 && Math.random() < 0.6) w.setBlock(x + dx, y, z + dz, B.torch, 0); }
  applySettings(s) {
  ...
  async quitToTitle() {
    this.ui.closeAllScreens(); await this.save(true); this.running = false;
    document.exitPointerLock && document.exitPointerLock();
    for (const w of this.worlds.values()) w.dispose(); this.worlds.clear();
    this.renderer.clearChunks(); this.entities.clear(); this.renderer.scene.remove(this.particles.points);
    for (const p of this.players) if (p.model) { this.renderer.scene.remove(p.model); p.model = null; }
    this.renderer.setViewCount(1); this.ui.reset(); this.players = [];
  ...
  frame(t) {
    ...
    this.pads.poll(dt);
    const paused = !!this.ui.globalScreen;
    if (paused) for (const pl of this.players) if (pl.input && pl.input.kind === 'gamepad') this.ui.gamepadGlobal(pl);
    if (!paused) {
      if (this.touch) this.touch.update();
      this.handleJoinRequests();
      // world & simulation
    ...
    // stream chunks around every player
    this.world.update(this.players.map(p => [p.x, p.z]), this.fps > 50 ? 7 : 4);
    // per-player camera + view state, then one render pass per view
    for (const p of this.players) this.updateCamera(p);
    const lead = this.player;
    const biome = this.world.biomeAt(lead.x, lead.z); const snow = biome.snow || (biome.temp < 0.15);
    this.renderer.weather.rain = this.world.dim === 0 && !biome.dry ? this.weather.intensity : 0;
    if (this.settings.fancyGraphics !== false) this.renderer.updateRain(dt, lead.view.camera.position, this.world, snow); else this.renderer.rain.visible = false;
    for (const p of this.players) this.updateViewState(p, dt);
    this.renderer.beginFrame();
    for (const p of this.players) {
      this.renderer.underwater = p.headInWater && p.thirdPerson === 0;
      this.renderer.inLava = p.headInLava && p.thirdPerson === 0;
      this.sunLevel = this.renderer.updateSky(this.time, p.view.camera.position, this.world.dim);
      this.renderer.renderView(p.view);
    }
    ...
    this.input.endFrame(); this.pads.endFrame();
  ...
  // An unclaimed controller pressing Start or A drops a new player into the game.
  handleJoinRequests() {
    if (this.settings.localMultiplayer === false) return;
    const claimed = this.players.map(p => p.input);
    if (this.players.length < 4) { const pad = this.pads.joinRequest(claimed); if (pad) { const p = this.addPlayer(pad); if (p) pad.rumble(200, 0.5, 0.3); } }
    // a controller player holding Back + B leaves
    for (const p of this.players.slice(1)) if (p.input.down('chat') && p.input.pressed('drop')) { if (p.input.suppressInput) p.input.suppressInput(20); this.removePlayer(p); }
  }
  updatePlayer(p, dt) {
    const inp = p.input, w = this.world;
    const focused = p.input.kind !== 'keyboard' || this.input.locked;
    // look
    if (focused && !p.screenOpen && !p.sleeping && !p.dead) {
      const l = inp.look(dt, this.settings.sensitivity);
      if (l.dx || l.dy) { p.yaw -= l.dx; p.pitch -= l.dy; p.pitch = Math.max(-Math.PI / 2 + 0.001, Math.min(Math.PI / 2 - 0.001, p.pitch)); }
    }
    // discrete actions
    if (focused) this.playerActions(p);
    // simulation
    if (w.isLoaded(Math.floor(p.x), Math.floor(p.z))) { const sub = dt > 0.034 ? 2 : 1; for (let i = 0; i < sub; i++) p.update(dt / sub); } else p.updateLook();
    // mining / using
    const canAct = focused && !p.screenOpen && !p.dead;
    p.mine(dt, canAct && inp.down('attack'));
    if (canAct && inp.down('use')) { p.useHold = (p.useHold || 0) + dt; if (p.useHold > 0.22) { p.useHold = 0; p.use(); } } else p.useHold = 0.2;
  }
  // Discrete button actions. Keyboard/mouse players are driven by UI.bindGlobal's key
  // handler instead, so only controllers are processed here.
  playerActions(p) {
    const inp = p.input;
    if (inp.kind !== 'gamepad') return;
    if (inp.pressed('pause')) { if (p.screenOpen) this.ui.closeScreen(p); else this.ui.openPause(p); return; }
    if (p.screenOpen) { this.ui.gamepadNavigate(p); return; }
    if (p.dead) { if (inp.pressed('jump') || inp.pressed('menuSelect')) { if (!this.hardcore) { this.ui.closeScreen(p); p.respawn(); } } return; }
    if (inp.pressed('inventory')) { this.ui.openInventory(p); return; }
    if (inp.pressed('drop')) this.dropHeld(p, false);
    if (inp.pressed('swap')) { const t = p.inventory.offhand[0]; p.inventory.offhand[0] = p.inventory.held; p.inventory.setHeld(t); this.ui.invalidateInventory(p); }
    if (inp.pressed('camera')) p.thirdPerson = (p.thirdPerson + 1) % 3;
    if (inp.pressed('chat') && p.input.kind === 'keyboard') this.ui.openChat(p, '');
    if (inp.pressed('hotbarNext')) p.inventory.selected = (p.inventory.selected + 1) % 9;
    if (inp.pressed('hotbarPrev')) p.inventory.selected = (p.inventory.selected + 8) % 9;
    const d = inp.hotbarDigit(); if (d >= 0) p.inventory.selected = d;
    if (inp.kind === 'gamepad') { if (inp.pressed('attack')) this.onMouseDown(0, p); if (inp.pressed('use')) { p.useHold = 0; this.onMouseDown(2, p); } }
  }
  updateViewState(p, dt) {
    const v = p.view; if (!v) return;
    const r = this.renderer;
    r.showSelection(v, p.selectedBox && !p.lookEntity && p.canBuild && !p.dead ? p.selectedBox : null);
    const held = p.inventory.held; r.setHeldItem(v, held ? held.id : 0, 0);
    const lightHere = this.world.getLightLevel(Math.floor(p.x), Math.floor(p.eyeY), Math.floor(p.z), this.sunLevel) / 15;
    r.updateHand(v, dt, p.moving && p.onGround, lightHere);
    r.setHandVisible(v, p.thirdPerson === 0 && !p.spectator && !p.dead);
    if (p.bowCharge >= 0) v.hand.position.z += p.bowCharge * 0.15;
    if (p.eating > 0) v.hand.rotation.x = -0.5 + Math.sin(p.eating * 30) * 0.1;
  }
  tick() {
    ...
    w.processTicks(1);
    for (const pl of this.players) w.randomTicks(Math.floor(pl.x), Math.floor(pl.z), 4, 3, this.rng);
    ...
    this.updateWeather();
    // sleeping
    const sleepers = this.players.filter(pl => pl.sleeping);
    if (sleepers.length && sleepers.length === this.livePlayers().length && sleepers.every(pl => pl.sleepT > 2.5)) { for (const pl of sleepers) { pl.sleeping = false; pl.sleepT = 0; } this.time = Math.floor(this.time / 24000) * 24000 + 24000 + 1000; this.daysSinceSleep = 0; if (this.weather.raining && Math.random() < 0.7) this.setWeather(false, false); this.ui.showToast('Good morning!'); }
    // autosave
    ...
    // End boss
    if (w.dim === 2 && !this.dragonKilled && (!this.bossEntity || this.bossEntity.removed) && this.players.some(pl => Math.hypot(pl.x, pl.z) < 160)) { this.bossEntity = this.entities.spawnMob('ender_dragon', 0, 85, -40, { noDespawn: true }); this.ui.showTitle('', 'The Ender Dragon awakens'); this.playSoundAt('dragon', 0, 85, -40); }
  }
  tickSpawner(te) {
    if (this.difficulty === 0) return;
    if (this.nearestPlayerDist(te.x, te.y, te.z) > 16) return;
  ...
  updateCamera(p) {
    ...
    this.ensurePlayerModel(p);
    v.hidden = [];
    if (p.thirdPerson === 0) {
      cam.position.set(ex, ey, ez);
      if (p.hurtTimer > 0.4) cam.rotation.z = (p.hurtTimer - 0.4) * 2;
      if (p.model) v.hidden = [p.model];
      this.animatePlayerModel(p);
      return;
    }
    const d = p.lookDir; const back = p.thirdPerson === 1 ? -1 : 1; const dist = 4;
    ...
    if (p.thirdPerson === 2) cam.rotation.set(-p.pitch, p.yaw + Math.PI, 0);
    if (dd <= 1.1 && p.model) v.hidden = [p.model];
    this.animatePlayerModel(p);
  }
  ensurePlayerModel(p) {
    if (p.model) return p.model;
    const skins = [{ skin: 0xd9a58a, hair: 0x4a2f1a, shirt: 0x2fb5c9, pants: 0x3c3a8f }, { skin: 0xc98a63, hair: 0x2a1a10, shirt: 0xd04a3a, pants: 0x33356b },
      { skin: 0xf0c9a0, hair: 0xc8a02a, shirt: 0x4ab84a, pants: 0x4a3a2a }, { skin: 0x8d5a3b, hair: 0x1a1210, shirt: 0xb04ac0, pants: 0x2a2a3a }];
    const c = skins[p.index % skins.length];
    p.model = humanoidModel({ skin: c.skin, hair: c.hair, shirt: c.shirt, pants: c.pants, sleeve: c.skin }, { key: 'player' + p.index, face: (ctx) => { ctx.fillStyle = '#' + c.hair.toString(16).padStart(6, '0'); ctx.fillRect(0, 0, 16, 3); ctx.fillStyle = '#fff'; ctx.fillRect(3, 6, 4, 2); ctx.fillRect(9, 6, 4, 2); ctx.fillStyle = '#3a4a8a'; ctx.fillRect(5, 6, 2, 2); ctx.fillRect(9, 6, 2, 2); ctx.fillStyle = '#a06050'; ctx.fillRect(6, 11, 4, 2); } });
    this.renderer.scene.add(p.model);
    return p.model;
  }
  animatePlayerModel(p) {
    const m = p.model; if (!m) return;
    m.visible = !p.spectator && !(p.dead && p.deadT > 1);
    m.position.set(p.x, p.y, p.z); m.rotation.y = p.yaw; m.userData.head.rotation.x = -p.pitch;
    const sp = Math.hypot(p.vx, p.vz); p._animT = (p._animT || 0) + (sp > 0.3 ? sp * 4 : 0) * 0.016;
    const sw = Math.sin(p._animT) * Math.min(1, sp / 3) * 0.8;
    m.userData.legs.forEach((l, i) => l.rotation.x = i ? -sw : sw); m.userData.arms.forEach((a, i) => a.rotation.x = i ? sw : -sw);
    if (p.sleeping) { m.rotation.x = -Math.PI / 2; m.position.y = p.y + 0.2; } else m.rotation.x = 0;
    const l = this.world.getLightLevel(Math.floor(p.x), Math.floor(p.y + 1), Math.floor(p.z), this.sunLevel) / 15;
    m.traverse(o => { if (o.isMesh) (Array.isArray(o.material) ? o.material : [o.material]).forEach(mm => { if (!mm.userData.base) mm.userData.base = mm.color.clone(); mm.color.copy(mm.userData.base).multiplyScalar(0.2 + 0.8 * l); }); });
  }
  ...
  // ---------- sound ----------
  playSoundAt(name, x, y, z, opts = {}) { const d = this.nearestPlayerDist(x, y, z); this.audio.play(name, Object.assign({ dist: d }, opts)); }

  // ---------- interactions ----------
  onMouseDown(button, p = this.player) {
    if (p.dead || p.screenOpen) return;
    if (p.sleeping) { p.sleeping = false; p.sleepT = 0; return; }
    if (button === 0) { if (p.lookEntity) p.attack(); else if (p.spectator) { } else { p.swing(); if (!p.lookHit && !p.creative) p.exhaustion += 0.01; } }
  ...
  dropHeld(p, all) { if (typeof p === 'boolean') { all = p; p = this.player; } const s = p.inventory.held; if (!s) return; const n = all ? s.count : 1; this.dropStack(stackOf(s, n), p); s.count -= n; if (s.count <= 0) p.inventory.setHeld(null); p.swing(); }
  dropStack(stack, p = this.player) { const d = p.lookDir; this.entities.dropItem(p.x + d[0] * 0.5, p.eyeY - 0.3, p.z + d[2] * 0.5, stack, false, [d[0] * 6 + p.vx, d[1] * 6 + 1, d[2] * 6 + p.vz]); this.ui.invalidateInventory(p); }
  openBlockUI(kind, x, y, z, p = this.player) {
    const w = this.world; const def = BLOCKS[w.getBlock(x, y, z)];
    if (kind === 'crafting') this.ui.openCrafting(p, x, y, z);
    else if (kind === 'chest') { const te = w.getTileEntity(x, y, z); if (te.lootKind) { fillLoot(te, te.lootKind, Math.random); delete te.lootKind; } this.ui.openChest(p, te, def.displayName); }
    else if (kind === 'furnace') { const te = w.getTileEntity(x, y, z); this.ui.openFurnace(p, te, def); }
    else if (kind === 'enchant') this.ui.openEnchant(p, x, y, z);
    else if (kind === 'anvil') this.ui.openAnvil(p, x, y, z);
  }
  hurtPlayer(amount, source, bypassArmor = false) { const t = source && source.target && source.target.isPlayer ? source.target : this.nearestPlayer(source ? source.x : this.player.x, source ? source.y : this.player.y, source ? source.z : this.player.z) || this.player; return t.hurt(amount, source, bypassArmor ? { fall: true } : {}); }
  onPlayerDeath(cause, p = this.player) { this.ui.chatMessage(p.name + ' ' + cause, '#ff5555'); this.ui.showDeath(p, cause); if (this.hardcore && p.index === 0) { this.meta.hardcoreDead = true; } if (p.input && p.input.rumble) p.input.rumble(400, 0.8, 0.6); this.save(false); }
  ...
  explode(x, y, z, power, source, fire = false) {
    ...
    for (const pl of this.players) hitE(pl, true); for (const e of this.entities.list) if (e !== source && !e.removed) hitE(e, false);
  }
  trySleep(x, y, z, p = this.player) {
    const w = this.world;
    if (w.dim !== 0) { w.setBlock(x, y, z, 0); this.explode(x + 0.5, y + 0.5, z + 0.5, 5, null, true); return; }
    p.bedSpawn = { x: x + 0.5, y: y + 1, z: z + 0.5, dim: 0 }; this.ui.showToastFor(p, 'Respawn point set');
    if (this.isDay() && !this.weather.thunder) { this.ui.showToastFor(p, 'You can only sleep at night'); return; }
    ...
    if (hostile) { this.ui.showToastFor(p, 'You may not rest now, there are monsters nearby'); return; }
  ...
  async changeDimension(dim, spawnOverride = null, initiator = null) {
    const p = initiator || this.player; if (this.switching) return; this.switching = true;
    ...
    const w = this.createWorld(dim); this.world = w; p.world = w; w._lastKey = null;
    ...
    for (const pl of this.players) { if (pl === p) continue; pl.x = tx + (Math.random() - 0.5) * 2; pl.y = ty; pl.z = tz + (Math.random() - 0.5) * 2; pl.vx = pl.vy = pl.vz = 0; pl.portalT = -3; pl.fallStart = null; if (pl.screen) this.ui.closeScreen(pl, true); }
    ...
    for (const pl of this.players) if (pl !== p) { pl.x = p.x + (Math.random() - 0.5) * 2; pl.z = p.z + (Math.random() - 0.5) * 2; pl.y = p.y; this.settleSpawn(pl); }
    ...
    this.switching = false; for (const pl of this.players) this.ui.invalidateInventory(pl);
  }
  ...
  async save(full) {
    ...
    const p = this.player;
    const meta = Object.assign({}, this.meta, { id: this.worldId, seed: this.seed, time: this.time, weather: { raining: this.weather.raining, thunder: this.weather.thunder, timer: this.weather.timer, intensity: this.weather.intensity }, difficulty: this.difficulty, cheats: this.cheats, hardcore: this.hardcore, dim: this.world.dim, player: p.serialize(), extraPlayers: this.players.slice(1).map(pl => pl.serialize()), lastPlayed: Date.now(), dragonKilled: this.dragonKilled, stats: this.stats, daysSinceSleep: this.daysSinceSleep, gamemode: p.gamemode });
    this.meta = meta;
